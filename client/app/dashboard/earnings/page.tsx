  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold text-white mb-2">Financial Overview</h1>
          <p className="text-muted-foreground">Track your revenue, payouts, and business growth.</p>
        </div>
        <button className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-colors shadow-lg shadow-emerald-500/20">
          Withdraw Funds
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }} className="bg-card/40 backdrop-blur-xl border border-white/5 p-6 rounded-3xl relative overflow-hidden">
          ...
          <h2 className="text-4xl font-black text-white">${stats.totalEarnings.toFixed(2)}</h2>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-card/40 backdrop-blur-xl border border-white/5 p-6 rounded-3xl relative overflow-hidden">
          ...
          <h2 className="text-4xl font-black text-white">${stats.pendingPayout.toFixed(2)}</h2>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-emerald-500 p-6 rounded-3xl relative overflow-hidden shadow-xl shadow-emerald-500/20">
          <div className="absolute top-0 right-0 p-6 opacity-10 text-white"><Activity className="w-24 h-24" /></div>
          ...
          <h2 className="text-4xl font-black text-white">{stats.completedOrders}</h2>
        </motion.div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-card/40 backdrop-blur-xl border border-white/5 p-6 md:p-8 rounded-3xl h-96 flex flex-col justify-center items-center text-center">
        ...
        <div className="bg-card/40 backdrop-blur-xl border border-white/5 p-6 rounded-3xl">
              ...
              {[1, 2, 3].map((_, i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-white text-sm">Payout Processed</p>
