...
export default function UserLoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push("/home"); // Redirect to Customer Homepage
    } catch (err: any) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/auth/verify-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone, otp, role: "user", name: "Phone User", email: `phone_${phone}@example.com` }),
      });
      const data = await res.json();
      ...
      >
        <div className="backdrop-blur-2xl bg-card/40 border border-zinc-200 dark:border-white/10 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center mb-6 relative z-10 flex flex-col items-center">
            ...
            <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-1">Customer Login</h2>
            <p className="text-muted-foreground text-sm">Welcome back to your local marketplace</p>
          </div>

          <div className="flex gap-2 mb-6 relative z-10">
            <button onClick={() => setMethod("email")} className={`flex-1 py-2 text-sm font-semibold rounded-lg border ${method === 'email' ? 'border-primary bg-primary/10 text-primary' : 'border-zinc-200 dark:border-white/10 text-muted-foreground'}`}>Email</button>
            <button onClick={() => setMethod("phone")} className={`flex-1 py-2 text-sm font-semibold rounded-lg border ${method === 'phone' ? 'border-primary bg-primary/10 text-primary' : 'border-zinc-200 dark:border-white/10 text-muted-foreground'}`}>Phone</button>
          </div>
            ...
            <form onSubmit={handleEmailLogin} className="space-y-4 relative z-10">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-black/20 border border-zinc-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary transition-all text-foreground" placeholder="name@example.com" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-black/20 border border-zinc-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary transition-all text-foreground" placeholder="••••••••" />
                </div>
              </div>

              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={isLoading} className="w-full py-3.5 px-4 text-zinc-900 dark:text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2 group disabled:opacity-50 disabled:cursor-not-allowed bg-primary shadow-[0_0_20px_rgba(139,92,246,0.4)] mt-2">
            ...
            <form onSubmit={otpSent ? handleVerifyOtp : handleSendOtp} className="space-y-4 relative z-10">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <input type="tel" required value={phone} disabled={otpSent} onChange={(e) => setPhone(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-black/20 border border-zinc-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary transition-all text-foreground disabled:opacity-50" placeholder="+1 234 567 8900" />
                </div>
              </div>
              ...
              {otpSent && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-2">
                  ...
                  <div className="relative group">
                    <MessageSquare className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
                    <input type="text" required value={otp} onChange={(e) => setOtp(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-black/20 border border-zinc-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary transition-all text-foreground" placeholder="123456" />
                  </div>
                  <p className="text-xs text-muted-foreground mt-1 ml-1 text-primary">Hint: Enter 123456</p>
                </motion.div>
              )}
